...
// on vérifie si order et column existent 
...
// on s'assure que le formulaire n'est pas vide
if(!empty($_POST)){
	foreach($_POST as $key => $value){
		$_POST[$key] = strip_tags(trim($value));
	}

	// vérifie que la longueur minimale des nom  et prénom de 3 caractères est bien respectée
	...
	// Vérifie la conformité de l'adresse email rentrée'
		if(!filter_variable($_POST['email'], FILTER_VALIDATE_EMAIL)){
		$errors[] = 'L\'adresse email est invalide';
	}
	// vérifie si les champs ont bien été remplis
	...
	// On vérifie si le formulaire comporte des erreurs
	if(count($errors) == 0){ 
...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<?php
				// confirmation de l'exécution de la requête
			...
			<div class="col-md-5">
				...
				<form method="post" class="form-horizontal well well-sm">
					<fieldset>
						...
						<div class="form-group">
							<div class="col-md-4 col-md-offset-4"><button type="submit" class="btn btn-primary">Envoyer</button>
							</div>
						</div>
					</fieldset>
				</form>
