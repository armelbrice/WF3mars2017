...
$app->get('/twig',function() use ($app){
    return $app['twig']->render('twig.html.twig',
            ['myvar'=>'Ma variable']
            );
})
->bind('twig')
...
// Utilisation du nouveau contrôleur dans une route
$app->get('/test','first.controller:testAction')
    ->bind('test')
;

// $name sera passé à la méthode testParamAction
$app->get('/test/{name}','first.controller:testParamAction')
    ->bind('test_param')
;

// 
$app->get('/users','first.controller:usersAction')
    ->bind('users')
;

$app->get('/user/{userId}','first.controller:userAction')
    ->bind('user')
;
