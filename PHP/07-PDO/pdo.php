...
/*
	exec() est utilisé popur formuler des requêtes ne retournant pas de jeu de résultats : INSERT, UPDATE et DELETE
...
// Attention : il faut choisir l'un des fetch que vous voulez exécuter sur un jeu de résultats, vous ne pouvez pas faire plusieurs fecth sur le même résultat n'en contenant qu'une seule. En effet, cette méthode déplace un curseur de lecture sur le résultat suivant contenu dans le jeu résultats : ainsi, quand il n'y en a qu'un seul, on sort du jeu.
...
// Quand vous faites une requête qui ne sort qu'un seul résultat : pas de boucle while, mais un fecth seul.
...
$resultat->bindParam(':nom', $nom, PDO::PARAM_STR);  // je lie le marque :nom à la variable $nom. Si on change le contenu de la variable, la valeur du marqueur changera automatiquement si on fait plusieurs execute
...
/*
	...
	Les requêtes préparées sont à préconiser si vous exécutez plusieurs fois la même requête (par exemple dans une boucle), et ainsi éviter le cycle complet analyse / interpratation / exécution de la requête.
...
// Si on change la valeur de la variable $nom, sans faire un nouveau bindValue, le marqueur de la requpete pointe toujours vers 'Thoyer' :
$nom = 'Durand';
$resultat->execute();
...
$resultat = $pdo->prepare("SELECT * FROM employes WHERE nom = ? AND prenom = ?");  // on prépare dans un premier temps, la requête sans sa partie variable, que l'on représenté avec des marqueurs sous forme de "?"
...
echo '<pre>'; print_r($resultat->errorInfo()); echo '</pre>';  // errorInfo() est une méthode qui appartient à la classe PDOStatment et qui fournit des infos sur l'erreur SQL éventuellement produite. On trouve le message de l'erreur à l'indice [2] de l'array retourné par cette méthode.
