...
function creationDuPanier() {
	if (!isset($_SESSION['panier'])) {
		//  s'il n'existe pas le panier dans SESSION, on le crée :
...
function ajouterProduitDansPanier($titre, $id_produit, $quantite, $prix) { //ces arguments sont en provenance de panier.php 
	...
	$position_produit = array_search($id_produit, $_SESSION['panier'] ['id_produit']); // array_search retourne un chiffre si l'id produit est présent dans l'array $_SESSION['panier'], qui correspond à l'indice auquel se situe l'élément (rappel : dans un array le premier indice vaut 0). Sinon retourne FALSE.
