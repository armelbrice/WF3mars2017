...
if (isset($_GET['id_produit'])) { // s'il existe l'indice id_produit dans l'url
    ...
    if ($resultat->rowCount() <= 0) {
        header('location:boutique.php'); // s'il n y a pas de résultat dans la requête, c'est que le produit n'existe pas : on oriente alors vers la boutique
        exit();
    }
...
while ($resultat = $requete->fetch(PDO::FETCH_ASSOC)) {
    $aside .= '<div class="col-sm-1 col-lg-1 col-md-1">';
        $aside .= '<div class="thumbnail">';
            $aside .= '<a href="fiche_produit.php?id_produit='. $resultat['id_produit'] .'"><img src="'. $resultat['photo'] .'" width="90" height="90" ></a>';
...
while ($resultat = $requete->fetch(PDO::FETCH_ASSOC)) {
    $aside .= '<div class="col-sm-3">';
        $aside .= '<a href="fiche_produit.php?id_produit='. $resultat['id_produit'] .'"><img src="'. $resultat['photo'] .'" style="width:100%"></a>';
