...
/*
	...
	Principe : un fichier temporaire appelé session est créé sur le serveur, avec un identifiant unique. Cette session est liée à un internaute car, dans le même temps, un cookie est déposé sur le poste de l'internaute avec l'identifiant. Ce cookie se détruit lorsqu'on quitte le navigateur. 
...
// session_destroy(); // suppression de la session MAIS il faut sa	voir que le session_destroy est d'abord vu par l'interpréteur qui ne l'exécute qu'à la fin du script

echo '<br> 3- La session après suppression totale : ';
echo '<pre>'; print_r($_SESSION); echo '</pre>';  // en effet, on voit envore le contenu de la session car la suppression n'intervient qu'à la fin du script. Pour s'en convaincre, vérifier que le fichier est supprimé dans le dossier xampp/tmp.
