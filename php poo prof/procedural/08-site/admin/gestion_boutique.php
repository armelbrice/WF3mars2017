...
if (!internauteEstConnecteEtEstAdmin()) {
	header('location:../connexion.php');  // si membre pas ADMIN, alors on le redirige vers la page de connexion qui est à la racine du site (en dehos du dossier admin)
	exit();
}
...
if ($_POST) { // équivalent à !empty($_POST) car si le $_POST est rempli, il vaut TRUE = formulaire posté
	...
	$contenu .= '<div class="bg-success">Le produit a été enregitré</div>';
		...
		while ($ligne = $resultat->fetch(PDO::FETCH_ASSOC)) {
			$contenu .= '<tr>';
				...
				$contenu .= '<td>
								...
								onclick="return(confirm(\'Etes-vous certain de vouloir supprimer ce produit ? \'));"       >supprimer</a>
							 </td>';
			$contenu .= '</tr>';
		}
	$contenu .= '</table>';
...
<form method="post" enctype="multipart/form-data" action=""><!-- "multipart/form-data" permet d'uploader un fichier et de générer une superglobale $_FILES -->
	...
	<input type="hidden" id="id_produit" name="id_produit" value="<?php echo $produit_actuel['id_produit'] ?? 0; ?>"><!-- champ caché qui réceptionne l'id_produit nécessaire lors de la modification d'un produit existant -->
