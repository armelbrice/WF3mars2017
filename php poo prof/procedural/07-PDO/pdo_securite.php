...
if(!empty($_POST)) {
	...
	//3- traitement contre les failles XSS (injection JS) ou les injections CSS : on parle d'échappement des données reçues :
	...
	$_POST['message'] = htmlspecialchars($_POST['message'], ENT_QUOTES);  // htmlspecialchars convertit les caractères spéciaux (<, >, ', ", &) en entités HTML (par exemple < devient &lt;), ce qui permet de rendre inoffensives les balises HTML. ENT_QUOTES permet d'ajouter les quotes à la liste des caractères convertis. 
	
	// En complément :
	$_POST['message'] = strip_tags($_POST['message']);   // permet de supprimer toutes les vbalises HTML contenu 
	
	// htmlentities() permet lui aussi de convertir les balises HTML en entités HTML
	
	
	//-------
	// 1- Nous allons faire une première requête qui n'est pas protégée contre les injections et qui n'accepte par les apostrophes :
