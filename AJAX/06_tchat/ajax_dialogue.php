...
if($mode == 'liste_membre_connecte' && empty($arg)) {
    ...
    if(!empty($arg)) { // si le message n'est pas vide. Alors on commence un INSERT INTO
...
while($message = $messages->fetch(PDO::FETCH_ASSOC)) {
    ...
    $contenu = str_replace($arg . '\n', "", $contenu); // on remplace le pseudo recherché par rien. l'antislash sert à faire remonter les prénoms après de l'un d'eux

    file_put_contents('prenom.txt', $contenu); // on écrase l'ancien contenu par un nouveau où l'on a supprimé le pseudo concerné
}
